...
% D3 (when local results land): the Qwen3-VL-8B Instruct/Thinking pair is served locally with
% vLLM from the official FP8 checkpoints — both variants FP8, so the same-weights ablation stays
% internally consistent. State this here; bf16 does not fit the 16 GB GPU used.
