...
Semantic scoring of the free-text explanations with embedding, entailment, and BERTScore measures replaces the near-zero $n$-gram picture and shows that mistyped detections still carry partially correct reasons.
\end{abstract}
...
\begin{enumerate}\setlength{\itemsep}{1pt}
  ...
  \item A \textbf{validity audit of agreement-based evaluation}: chance-corrected agreement statistics, per-model label priors, confusion structure with uncertainty estimates, a majority-vote ensemble scored against ground truth, and semantic (embedding, entailment, BERTScore) scoring of the free-text explanations.
\end{enumerate}
...
Fourth, the typed disagreement overstates the semantic disagreement: explanations attached to the wrong type are nearly as aligned with the human references as those attached to the right one, so models often perceive the correct reason and file it under the wrong code (Sec.~\ref{sec:semantic}).
...
The social and safety codes encode a particular normative frame --- the Awkward-vs-Forbidden boundary is culturally contestable --- and we query closed, drifting API endpoints. 
